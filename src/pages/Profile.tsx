...
const Profile = () => {
    ...
    useEffect(() => {
        const loadUserData = async () => {
            try {
                ...
                const userRatings = await ratingService.getUserRatings(session.user.id);
    ...
    const handleRatingChange = async (movieId: string, newRating: number) => {
        try {
            ...
            const filteredRatedMovies = ratedMoviesWithDetails.filter(Boolean);
            setRatedMovies(filteredRatedMovies);
            setStats(calculateStats(filteredRatedMovies));
        } catch (error) {
    ...
    if (!user) {
        return <Box sx={{ color: 'white', p: 4 }}>Aucun utilisateur connecté</Box>;
    }
